...
The proposed system received an overall ``Agree'' rating for Usability (WM = 3.50) and Data Security (WM = 3.53), while Reliability fell within ``Somewhat Agree'' (WM = 3.18). This indicates favorable but cautious acceptance, especially regarding the system's expected real-world performance.

Usability had the second-highest weighted mean (3.50, Agree). Respondents were most confident in completing tasks with minimal effort (Item 3.14, WM = 3.69) and in the system's ease of learning and navigation (Item 3.13, WM = 3.56). However, they rated the design's fit for their digital skill level lowest (Item 3.16, WM = 3.01, Somewhat Agree). This gap suggests that while the system is expected to be functional, some respondents remain unsure if it will accommodate varying digital literacy levels --- consistent with documented literacy barriers in rural Philippine telehealth studies \cite{ref6}, \cite{ref7}.

Reliability was the lowest and most inconsistent dimension (WM = 3.18). Respondents were confident the system would work without errors or crashes (Item 3.17, WM = 4.50) and be consistently available (Item 3.19, WM = 4.03). However, confidence dropped for connectivity-dependent items: usability during poor connectivity (Item 3.18, WM = 3.41) and, notably, accurate and timely notifications (Item 3.20, WM = 2.33, Slightly Agree) --- the lowest score overall. This suggests respondents distinguish between the system's core stability, viewed favorably, and its reliance on external factors like connectivity and message delivery, viewed more skeptically. This aligns with the connectivity and infrastructure barriers noted earlier as unresolved constraints in Barangay Panicuason, reinforcing that skepticism stems from lived experience rather than unfamiliarity with telemedicine.

Data Security scored highest overall (WM = 3.53) but showed a similar spread. Respondents strongly believed the system would restrict access to authorized personnel (Item 3.22, WM = 4.21) and include secure login authentication (Item 3.24, WM = 4.24). However, trust in the system keeping health information private (Item 3.21, WM = 3.05) and comfort in providing personal information (Item 3.23, WM = 2.78) were rated lower.

Overall, these findings suggest that confidence in technical security mechanisms does not fully translate into personal trust --- consistent with the Technology Acceptance Model's distinction between perceived usefulness and the more subjective, trust-based dimensions of adoption \cite{ref56}.
% \chapter*{Results and Discussions}
